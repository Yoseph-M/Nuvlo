...
export const sendPasswordResetEmail = async (
  ...
  const mailOptions = {
    from: `"Nuvlo" <${process.env.FROM_EMAIL || "nulvo.bet@gmail.com"}>`,
    to: email,
    subject: "Reset Your Password - Nuvlo",
    html: `
      <div style="font-family: 'Outfit', 'Inter', sans-serif; background-color: #faf9f6; padding: 40px; color: #141414;">
        <div style="max-width: 580px; margin: 0 auto; background-color: #ffffff; border: 1px solid #e5e5e0; padding: 40px; border-radius: 8px; box-shadow: 0 4px 12px rgba(20, 20, 20, 0.03);">
          <!-- Logo Header -->
          ...
          <h2 style="font-size: 20px; font-weight: 500; margin-bottom: 20px; text-align: center;">Reset Password Request</h2>
          
          <p style="font-size: 14px; line-height: 1.6; color: #5a5a54; margin-bottom: 25px; text-align: center;">
            Hello ${name}, we received a request to reset your password. Click the link below to set a new password. If you didn't request this, you can ignore this email.
          </p>
          
          <div style="text-align: center; margin-bottom: 30px;">
            <a href="${resetUrl}" style="display: inline-block; background-color: #141414; color: #faf9f6; text-decoration: none; padding: 12px 30px; font-size: 12px; font-weight: 500; letter-spacing: 0.15em; text-transform: uppercase; border-radius: 4px; transition: background-color 0.2s;">
              Reset Password
            </a>
          </div>
          ...
          <p style="font-size: 12px; line-height: 1.6; color: #8a8a80; text-align: center; margin-bottom: 0;">
            Or copy and paste this link in your browser:
            <br />
            <a href="${resetUrl}" style="color: #141414; text-decoration: underline; word-break: break-all;">${resetUrl}</a>
          </p>
  ...
  try {
    ...
    throw new Error("Could not send password reset email. Please check SMTP settings.");
  }
};
